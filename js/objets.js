class Objets {
  ...
  constructor(typeObjet, objets) {
    this.typeObjet=typeObjet;
    this.objets=objets;

    if (typeObjet in localStorage) {
      this.objets = JSON.parse(localStorage.getItem(typeObjet));;
    } else {      
      this.objets = objets;
      this.save();
    }
  ...
  actionButton(objet) {
    ...
    "<div class='dropdown'> \
      ...
      <ul class='dropdown-menu'> \
        <li><button id='btn-acheter-"+objet.nom.id()+"' type='button' class='btn' data-bs-toggle='modal' data-bs-target='#action-modal'>Acheter</button> \
        <li><button id='btn-marchander-"+objet.nom.id()+"' type='button' class='btn' data-bs-toggle='modal' data-bs-target='#action-modal'>Marchander</button> \
        <li><button id='btn-voler-"+objet.nom.id()+"' type='button' class='btn' data-bs-toggle='modal' data-bs-target='#action-modal'>Voler</button> \
      </ul> \
    </div>";
  ...
  setActionButton(objet) {
    //<td>"+self.actionButton(objet)+"</td> \
...
class Armures extends Objets{
  constructor(armures) {
    super("armures", armures);
  }

  liste() {

    let type="";
    let tId="#t-"+this.typeObjet;
    let self=this;
    ...
    this.objets.forEach(function(objet) {
      if (type != objet.type) {
        ...
        let strLigne=" \
        <tr> \
          <td colspan='5' style='background-color: burlywood;' onclick='$(\".c-"+objet.type.id()+"\").toggle()'><i class='"+objet.icon+"'><span style='font-weight: bold;'> "+objet.type+"</span></i></td> \
        </tr>";
        $(tId).append(strLigne);    
      }

      let strLigne =
      "<tr id='t-"+objet.nom.id()+"' class='c-"+objet.type.id()+"' style='display: none;'> \
        <td>"+objet.nom+"</i></td> \
        <td>"+objet.prix+"</td> \
        <td>"+objet.pr+"</td> \
        <td>"+objet.rupture+"</td> \
